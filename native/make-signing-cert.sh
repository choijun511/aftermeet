...
# 用一张固定的自签名证书签名后,TCC 把授权绑定到证书身份,减少重新打包导致的授权失效（首次切换身份仍需重新授权）。
#
...
openssl pkcs12 -export -inkey key.pem -in cert.pem -out id.p12 -passout env:CERT_PASSWORD -name "${NAME}" >/dev/null 2>&1

echo "[cert] 导入登录钥匙串(仅允许 /usr/bin/codesign 使用私钥)…"
security import id.p12 -k ~/Library/Keychains/login.keychain-db -P "$CERT_PASSWORD" -T /usr/bin/codesign
