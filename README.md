...
- **录音回放**：会议详情中播放、拖动、倍速和前后 10 秒；有可靠时间戳的转写片段可点击回听，旧记录和本地文字支持整段回放。回放在本机完成，播放期间暂停自动起录。
...
设置页统一选择「仅实时字幕」「本地精转」或「云端精转」。云端模式会上传音频至阿里云并可能计费；缺少密钥时界面明确显示本地回退。检测到密钥仅代表已配置，不代表服务连接已验证。

会议详情优先展示结论和待办；逐句回听、技术详情及已完成任务的恢复工具按需展开。主要控件支持 Tab 聚焦及 Enter/空格操作，弹窗支持 Esc 关闭并恢复焦点。
